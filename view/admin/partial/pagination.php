...
<div class="flex flex-col items-center justify-center my-8">
    ...
    <ul class="inline-flex items-center space-x-1">
        <?php if ($current > 1): ?>
            <li><a class="px-3 py-1 rounded bg-gray-100 hover:bg-gray-200 text-gray-700" href="<?php echo $baseUrl; ?>?page=<?php echo ($current - 1); ?>">Trước</a></li>
        <?php endif; ?>

        <?php for ($i = max(1, $current - 2); $i <= min($total, $current + 2); $i++): ?>
            <li>
                <a class="px-3 py-1 rounded <?php echo ($i === $current) ? 'bg-blue-500 text-white font-bold' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'; ?>" href="<?php echo $baseUrl; ?>?page=<?php echo $i; ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>

        <?php if ($current < $total): ?>
            <li><a class="px-3 py-1 rounded bg-gray-100 hover:bg-gray-200 text-gray-700" href="<?php echo $baseUrl; ?>?page=<?php echo ($current + 1); ?>">Sau</a></li>
        <?php endif; ?>
    </ul>
</div>